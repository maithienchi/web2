...
router.post('/Request', asyncHandler(async function (req, res) {
    ...
    found.addToken();
    await Email.send(found.email, 'Quên mật khẩu - SalaBank <no-reply>', `<b>Quý khách vui lòng không chia sẻ link cho bất kỳ ai để tránh mất tài khoản!</b><br>Click vào đường dẫn để đặt lại mật khẩu: <a href="${process.env.BASE_URL}/resetpassword/userId/${found.id}/${found.token}">${process.env.BASE_URL}/resetpassword/userId/${found.id}/${found.token}</a>`);
